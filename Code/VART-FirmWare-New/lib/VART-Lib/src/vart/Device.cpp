...
const vart::DeviceSettings vart::settings = {
    .servomotor = {
    ...
    .planner = Planner(
        settings.planner,
        {
            ...
            .left_pulley = Pulley(
                ...
                hardware::ServoMotor(
                    settings.servomotor,
                    hardware::MotorDriverL293(LeftDriverA, LeftDriverB),
                    hardware::Encoder(LeftEncoderA, LeftEncoderB)
                )
            ),
            .right_pulley = Pulley(
                settings.pulley,
                hardware::ServoMotor(
                    settings.servomotor,
                    hardware::MotorDriverL293(RightDriverA, RightDriverB),
                    hardware::Encoder(RightEncoderA, RightEncoderB)
                )
            )
        }
    ),
