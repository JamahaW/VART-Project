...
size_t OLED::write(uint8_t data) {
    ...
    for (uint8_t offset = 0; offset < width_6; offset += OLED_FONT_WIDTH, cursor_x += font_width) {
        col = OLED_FONT_GET_COL(bits, offset);
        sendByte(col);
    }
...
void OLED::sendCommand(uint8_t cmd1) {
    beginOneCommand();
    Wire.write(cmd1);
    endTransmission();
}

void OLED::sendTwoCommands(uint8_t cmd2) {
    beginCommand();
    Wire.write(129);
    Wire.write(cmd2);
    endTransmission();
...
void OLED::setWindow(uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1) {
    beginCommand();
...
void OLED::beginCommand() {
    beginTransmission(OLED_COMMAND_MODE);
}

void OLED::beginOneCommand() {
    beginTransmission(OLED_ONE_COMMAND_MODE);
}

void OLED::endTransmission() {
    Wire.endTransmission();
    writes = 0;
}

void OLED::beginTransmission(uint8_t mode) const {
